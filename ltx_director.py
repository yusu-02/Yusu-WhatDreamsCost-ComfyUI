...
def _build_combined_audio(timeline_data_str: str, start_frame: int, duration_frames: int, frame_rate: float, override_audio: bool = False) -> dict:
    ...
    peak = out_waveform.abs().max()
    if peak.item() > 1.0:
        out_waveform = out_waveform / peak

    return {"waveform": out_waveform.unsqueeze(0), "sample_rate": target_sr}


def _ltxv_pixel_frames(duration_frames: int) -> int:
    return max(9, int(math.ceil((max(1, int(duration_frames)) - 1) / 8.0) * 8) + 1)
...
class LTXDirector(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            node_id="YusuLTXDirector",
            display_name="Yusu LTX Director",
            ...
            inputs=[
                ...
                io.String.Input(
                    "transition_smoothness", default="", optional=True,
    ...
    def execute(cls, model, clip, start_second, end_second, duration_seconds, start_frame, end_frame, duration_frames,
                timeline_data, local_prompts, segment_lengths, global_prompt="", guide_strength="", transition_smoothness="", epsilon=1e-3,
        ...
        if optional_latent is None:
            ...
            # LTXV decodes T temporal latents to T*8+1 pixel frames.
            latent_t = _ltxv_latent_frames(ltxv_length)
            samples = torch.zeros(
