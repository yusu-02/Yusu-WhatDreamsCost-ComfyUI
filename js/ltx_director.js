...
class TimelineEditor {
  ...
  // The timeline only ever grows 閳?never shrinks 閳?through this method.
  ...
  // the furthest segment end (across both tracks) 鑴?1.30, with a floor of getDurationFrames().
  // This is used for all rendering/positioning 閳?the actual output duration is getDurationFrames().
  ...
  createDOM() {
    ...
    this.audioTrimLabel = document.createElement("span");
    this.audioTrimLabel.className = "pr-duration-label";
    this.audioTrimLabel.textContent = "Trim Start (s):";
    this.audioTrimLabel.style.display = "none";
    ...
    this.audioTrimValue.addEventListener("change", (e) => {
      this.setSelectedAudioTrimStartSeconds(e.target.value);
    });
    this.audioTrimValue.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        this.setSelectedAudioTrimStartSeconds(e.target.value);
        this.audioTrimValue.blur();
      }
    });

    this.audioCutBtn = document.createElement("button");
    this.audioCutBtn.className = "pr-icon-btn";
    this.audioCutBtn.style.padding = "4px";
    this.audioCutBtn.innerHTML = ICONS.scissors;
    this.audioCutBtn.title = "Cut selected audio at the playhead";
    this.audioCutBtn.style.display = "none";
    this.audioCutBtn.addEventListener("click", () => this.splitSelectedAudioAtPlayhead());
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          img.onload = () => {
            ...
            // Use the full intended length 閳?the timeline has already been grown to fit.
    ...
    for (let file of files) {
      ...
      await new Promise(async (resolve) => {
        try {
          ...
          // Use the full clip length 閳?timeline has already grown to fit.
          let constrainedLength = newLength;
  ...
  syncDurationToTimelineSegments(reflowImageText = true) {
    ...
    if (this.durationSecondsWidget) {
      this.durationSecondsWidget.value = parseFloat((totalFrames / frameRate).toFixed(3));
    }
  ...
  fitImageSegmentsToAudio(audioSeg, imageSegs) {
    const segments = (imageSegs || [])
      .filter(Boolean)
      .sort((a, b) => a.start - b.start);
    if (!audioSeg || segments.length === 0) return 0;

    if (segments.length === 1) {
      this.alignImageSegmentToAudio(segments[0], audioSeg);
      return 1;
    }

    const sourceLength = Math.max(1, segments.reduce((sum, seg) => sum + Math.max(MIN_SEGMENT_LENGTH, seg.length), 0));
    const audioEnd = audioSeg.start + audioSeg.length;
    let cursor = audioSeg.start;

    for (let i = 0; i < segments.length; i++) {
      const seg = segments[i];
      const remainingCount = segments.length - i - 1;
      const remainingReserve = remainingCount * MIN_SEGMENT_LENGTH;
      const maxLength = Math.max(MIN_SEGMENT_LENGTH, audioEnd - cursor - remainingReserve);
      let newLength;

      if (i === segments.length - 1) {
        newLength = Math.max(MIN_SEGMENT_LENGTH, audioEnd - cursor);
      } else {
        const ratioLength = Math.max(MIN_SEGMENT_LENGTH, Math.round((Math.max(MIN_SEGMENT_LENGTH, seg.length) / sourceLength) * audioSeg.length));
        newLength = clamp(ratioLength, MIN_SEGMENT_LENGTH, maxLength);
      }
  ...
  alignSelectedImageToOverlappingAudio(imageSeg) {
    ...
    const overlappingImages = this.getOverlappingImageSegments(audioSeg);
    this.fitImageSegmentsToAudio(audioSeg, overlappingImages.length > 1 ? overlappingImages : [imageSeg]);
    this.timeline.segments.sort((a, b) => a.start - b.start);
    this.selectionType = "image";
    this.selectedIndex = this.timeline.segments.findIndex(s => s.id === selectedId);
    this.syncDurationToTimelineSegments(false);
  ...
  updateUIFromSelection() {
    ...
    if (this.selectionType === "audio" && seg) {
      if (this.propContainer) this.propContainer.style.display = "none";
      ...
      this.strengthValue.value = "1.00";
      this.strengthValue.disabled = true;
      this.strengthValue.title = `File: ${seg.fileName || "Unknown"} | Source: ${this.formatTime(clipFrames)} | Trim-out: ${this.formatTime(Math.round(trimOutFrames))}`;
      if (this.audioTrimLabel) this.audioTrimLabel.style.display = "inline";
      if (this.audioTrimValue) {
        this.audioTrimValue.style.display = "inline-block";
        this.audioTrimValue.value = (trimStartFrames / frameRate).toFixed(2);
      }
      if (this.audioCutBtn) this.audioCutBtn.style.display = "inline-flex";
      if (this.durationValue) {
      ...
      if (this.propContainer) this.propContainer.style.display = "flex";
      this.audioInfoArea.style.display = "none";
      this.promptInput.style.display = "block";
      this.strengthRow.style.display = "flex";
      if (this.audioTrimLabel) this.audioTrimLabel.style.display = "none";
      if (this.audioTrimValue) this.audioTrimValue.style.display = "none";
      if (this.audioCutBtn) this.audioCutBtn.style.display = "none";
  ...
  render() {
    ...
    for (let i = 0; i < sortedSegments.length; i++) {
      ...
      if (imgObj && imgObj.complete && imgObj.naturalWidth > 0 && seg.type !== "ghost") {
        ...
        if (imgRatio > boxRatio) {
          // Fits width, vertical letterboxing (black bars top/bottom) 閳?keep as is
          this.ctx.drawImage(imgObj, drawX, drawY, drawW, drawH);
        } else {
          // Fits height, horizontal letterboxing (black bars left/right) 閳?tile horizontally
          this.ctx.drawImage(imgObj, drawX, drawY, drawW, drawH);
  ...
  commitChanges(skipRender = false) {
    ...
    for (let seg of sortedSegments) {
      ...
      if (seg.start > currentCursor) {
        // Gap between the cursor and this segment 閳?clip it at the cutoff too.
  ...
  showContextMenu(clientX, clientY, seg, trackType) {
    ...
    if (trackType === "audio") {
      ...
      if (overlappingImages.length > 0) {
        ...
        alignImagesBtn.innerHTML = `Fit Overlapping Image(s) to Audio`;
      ...
      if (overlappingAudio) {
        ...
        alignToAudioBtn.innerHTML = `Fit Segment to Audio`;
...
app.registerExtension({
  name: "Yusu-LTXDirector",
  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name === "Yusu-LTXDirector") {
